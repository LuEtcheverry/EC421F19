...
- Nonstationarity, 'in-class' analysis
- **Follow up:** .mono[EC422] (time series) is offered in the winter! 😃
...
**Assignment** due Sunday. Another one coming soon, due before Thanksgiving.
---
layout: true
# .mono[R] showcase
---
...
.hi-slate[Your responses:] Percentage who said .mono[TRUE] (74 responses)

```{R, guesses_y1, echo = F}
#Enter how many people say true for each combination. Go to Statistic on Canvas
data.table(c(85,8,73,12,91,24,22,23,16), c(16,72,8,7,28,86,34,96,32)) %>%
setnames(c("y1", "y2")) %>%
  t() %>%
  data.frame() %>%
  knitr::kable() %>%
